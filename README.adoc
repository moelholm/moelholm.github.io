...
To connect to an AWS account
    ...
    export AWS_PROFILE=codespace-sso

    # To refresh expired token:
    aws sso login --profile codespace-sso --no-browser
...
* **Comments:** Uses link:GISCUS_SETUP.md[giscus] (GitHub Discussions-based commenting system) - see link:GISCUS_SETUP.md[setup guide]
